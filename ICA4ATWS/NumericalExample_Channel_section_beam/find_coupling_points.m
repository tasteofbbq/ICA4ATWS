...
for loop_coup_info = 1:length(coupling_information)
    ...
    switch local_surface_index1
        case 1
            ...
            v = 0;
            [wGS, wW] = Gausspoint(-1,0,num_GS_Thi);
            ...
            for ii = 1:size(u,2)-1
                ...
                for numGP = 1:num_GS
                    ...
                    for qw = 1:num_GS_Thi
                        ...
                        [found_para, distance] = find_BFGS(X,Skin_total{patch_index2},local_surface_index2);
                        w1 = w(numGP);
                        w2 = wW(qw,1);
                        para = [s(numGP) v gs1];
        ...
        case 4
            ...
            for ii = 1:size(v,2)-1
                ...
                for numGP = 1:num_GS
                    ...
                    [~, ~ ,v31 ,theta1] = localCoordinate(SKL1);
                    [t1,ts1,tt1] = Thickness([u,s(numGP)],0,Skin_total{patch_index1});
                    Cps1 = [conpsX1(Rindex1),conpsY1(Rindex1),conpsZ1(Rindex1)];
                    for qw = 1:num_GS_Thi
                        gs1 = wGS(qw);
                        X = R1*Cps1+gs1*v31'*(t1/2);
